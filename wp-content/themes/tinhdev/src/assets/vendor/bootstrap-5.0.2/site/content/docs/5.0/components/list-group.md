...
Used in combination with `$theme-colors` to generate the [contextual variant classes](#contextual-classes)
for `.list-group-item`s.
...
You can activate a list group navigation without writing any JavaScript by simply specifying `data-bs-toggle="list"` or
...
Activates a list item element and content container. Tab should have either a `data-bs-target` or an `href` targeting a
